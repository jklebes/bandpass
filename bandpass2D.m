function image_out = bandpass2D(image, low_cutoff, high_cutoff, varargin)
...
if p.Results.filter=='gaussian'
    ...
    mask=gaussianMask(masksize_x, masksize_y, low_cutoff, high_cutoff);
...
function mask = gaussianMask(masksize_x, masksize_y, low_cutoff, high_cutoff)
    ...
    %array of distances
    dist=sqrt(xs.^2+ys.^2);
    mask_low = exp(-.005*dist);
    mask_high = exp(-4*dist);
